...
DEFINE toUpper edu.rosehulman.olsonmc.Upper();
...
neededFields = foreach songs generate title;

tokenized = foreach neededFields generate TOKENIZE(toUpper(title)) as L;
flat = foreach tokenized generate FLATTEN(L) as f;
result = Group flat by f;
resultCounted = foreach result generate group,COUNT(flat) as c;
ordered = Order resultCounted by c DESC;
limited = Limit ordered 100;

store limited into '$outputLoc';
